...
////   http://lampsvn.epfl.ch/svn-repos/scala/scala/trunk/src/actors/scala/actors/Actor.scala@16831
////
...
// $Id: Actor.scala 16831 2009-01-03 15:09:03Z phaller $
...
object Actor {
  ...
  /* JPF MODIFIED: Removed Timer Thread because time is not simulated by JPF 
   * (method timeout is handled, look the relevant methods for more insights)
   */
  ...
  def self: Actor = {
    ...
    if (null eq a) {
      // sanity check (but should never enter here)
      error("An actor should always be linked to a thread in the current JPF extension")
   ...
   */
  def resetProxy {
    println("DEBUG: `resetProxy` has no effect for the moment: FIX?!")
    /*
   ...
   */
  def clearSelf {
    println("DEBUG: `clearSelf` has no effect because we do not garbage collect actors")
    /*
    tl.set(null)
    */
  }
...
trait Actor extends AbstractActor {
  ...
  import gov.nasa.jpf.actor.adapter.scala.{ScalaActor,ScalaMsg}
  private def getInternalFrameworkActor(): ScalaActor =
    ScalaActor.getActor(this)
  /*
  ...
  */
  ...
  /* JPF MODIFIED: We removed the MessageQueue. However, this is a problem
   * because the queue is visible to actors, and therefore the methods defined in
   * it should be updated so that they can be correctly emulated by the internal
   * of the framework. TODO
   */
  ...
  */
  ...
  def mailboxSize: Int = synchronized {
    getInternalFrameworkActor().getMailboxSize()
  }

  /**
   * JPF MODIFIED: reroute the message into the framework's Cloud
   * 
  ...
  def send(msg: Any, replyTo: OutputChannel[Any]) = {
    val thisActor = getInternalFrameworkActor()
    thisActor.send(ScalaActor.getActor(replyTo.receiver), msg.asInstanceOf[Object],
        thisActor, replyTo);
  }
  ...
  def receive[R](f: PartialFunction[Any, R]): R = {
    val actor = getInternalFrameworkActor()
    assert(Actor.self == this, "receive from channel belonging to other actor")
    
    actor.waitingFor(f.isDefinedAt _)
    actor.getAttachedActorThread().yieldToPlatform();
    
    if (actor.shouldExit()) exit() // links
    
    val qel = actor.getReceived().asInstanceOf[ScalaMsg] 
        
    val received = Some(qel.getValue()) 
    
    sessions = qel.getReplyChannel().asInstanceOf[OutputChannel[Any]] :: sessions 
  ...
  def receiveWithin[R](msec: Long)(f: PartialFunction[Any, R]): R = {
    val thisActor = getInternalFrameworkActor()
    assert(Actor.self == this, "receive from channel belonging to other actor")
    assert(msec >= 0, "IllegalArgument: Negative delay.")  

    thisActor.waitingFor(f.isDefinedAt _)
    thisActor.send(thisActor, TIMEOUT$.MODULE$, thisActor, this);
    thisActor.getAttachedActorThread().yieldToPlatform();
    
    if (thisActor.shouldExit()) exit() // links
    
    var received: Option[Any] = None
    val qel = thisActor.getReceived().asInstanceOf[ScalaMsg] 
  ...
  def react(f: PartialFunction[Any, Unit]): Nothing = {
    ...
    val qel = actor.getReceived().asInstanceOf[ScalaMsg]  //mailbox.extractFirst((m: Any) => f.isDefinedAt(m))
    sessions = List(qel.getReplyChannel().asInstanceOf[OutputChannel[Any]]) //XXX: EVIL
    scheduleActor(f, qel.getValue())
  ...
  def reactWithin(msec: Long)(f: PartialFunction[Any, Unit]): Nothing = {
    ...
    val qel = thisActor.getReceived().asInstanceOf[ScalaMsg]  //mailbox.extractFirst((m: Any) => f.isDefinedAt(m))
    
    if ((TIMEOUT eq qel) && msec == 0) {
      if (f.isDefinedAt(TIMEOUT)) {
        sessions = List(this)
  ...
  def !?(msg: Any): Any = {
    ...
    send(msg, replyCh)
    val actor = ScalaActor.getActor(Actor.self)
    actor.beWaitingOnReply()
    replyCh.receive {
      case x => x
    }
  }
  ...
  def !!(msg: Any): Future[Any] = {
    error("Future operator `!!` need to be implemented")
    /*
  ...
  */
  ...
  protected def scheduleActor(f: PartialFunction[Any, Unit], msg: Any) =
    if(f eq null)
      error("Scheduling actor with null function. This should never happen.")
    else 
      f(msg)
  ...
  */
  ...
  def start(): Actor = synchronized {
    val a = getInternalFrameworkActor()
    assert(a != null, "Can't start a null actor")
    ...
    exitReason = 'normal
    exiting = false
    a.setShouldExit(false);

    if (!a.isSuspended())
      throw new UnsupportedOperationException(
          "Multiple starts still need to be implemented");
  ...
  private def seq[a, b](first: => a, next: => b): Unit = {
    ...
    */
      ...
      val cont: PartialFunction[Any,Unit] = { case _ => next }
      getInternalFrameworkActor().setContinuation(cont)
      //throw new SuspendActorException
    //}
    first
    ...
    def apply[T](f: => T): Unit = {
      try {
        ...
        case s: SuspendActorException => // do nothing
      }
    }
