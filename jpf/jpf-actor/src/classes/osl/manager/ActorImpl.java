...
public abstract class ActorImpl extends CoreActorThread implements Serializable {
   ...
   * @return <tt>true</tt> if the message is a valid "asynchException" message,
  ...
  /**
   * Build an asynchException message. It is still up to the caller to correctly
   * stamp the message after it has been built.
   * 
   ...
   *         message with the given <b>sender</b>, <b>receiver</b>, <b>cause</b>
   *         and exception <b>E</b>.
   */
  ...
  /**
   * Register an extension method. Note that this must be done before the method
   * may be called by internal actors. Moreover, extension methods must be
   * re-registered after every migration since instances of <em>Method</em> are
   * not serializable. Note that ALL variants of the given method name are
  ...
  protected void registerExtension(String extensionName) {
    ...
    for (i = 0; i < theMeths.length; i++)
      if (theMeths[i].getName().equals(extensionName)) {
        newMeths.insertElement(new MethodStructure(theMeths[i], theMeths[i]
            .getParameterTypes()));
      }
   ...
   *              Thrown if the class of new requested actor is not a subclass
   *              of <em>Actor</em>, or if the class of the new requested actor
   *              implementation is not a subclass of <em>ActorImpl</em>.
   * @exception osl.manager.RemoteCodeException
   *              Thrown for any other error encountered while attempting to
   *              perform the create. Note that this error may also be thrown
   *              asynchronously (e.g. when performing a remote create).
   */
   ...
   * @exception osl.manager.RemoteCodeException
   *              Thrown for any other error encountered while attempting to
   *              perform the send. This usually indicates a local error in the
   *              manager implementation rather than an error encountered at the
   ...
   *        called.
   * @param <b>thrower</b> The <em>ActorImpl</em> which is signalling the fatal
   *        error.
   ...
   *        called.
   * @param <b>thrower</b> A reference to the <em>ActorImpl</em> which wishes to
   *        be migrated.
   * @param <b>where</b> The <em>ActorManagerName</em> of the manager where this
   *        actor should be moved.
   * @exception osl.manager.IllegalNodeException
   *              Thrown if the <em>where</em> argument does not correspond to a
   *              legal <em>ActorManagerName</em>.
   ...
   *              exception. The remote copy is destroyed in this case and the
   *              actor is treated as if it never migrated.
   */
   ...
   * reference giving the name of the new actor, and a create request describing
   * the new <em>Actor</em> that should be created and managed. After calling
   * this method, a manager will call the run method in this class to start the
   * actor executing. Note that the protection of this method prevents it from
   * being called external to <tt>osl.manager</tt>. This was done to prevent
   * arbitrary classes from initializing new actors. However, managers in
   * different packages may invoke this method using <em>implInitialize</em>.
   * <p>
   * 
   ...
   *        the new actor.
   * @param <b>rtClass</b> The run-time <em>Class</em> of the user-written actor
   *        that should be instantiated by this implementation. This class will
   *        always be an extension of the <em>Actor</em> class.
   * @param <b>initArgs</b> The array of arguments to pass to the constructor of
   *        the user-defined actor when it is instantiated.
   * @exception java.lang.Exception
   *              The actual type of exception thrown depends on the
   *              implementation being used.
   * @see osl.manager.ActorManager#implInitialize
   */
  public abstract void actorInitialize(ActorManager ourMgr, ActorName you,
   ...
   * be sent back to the original caller using an "asynchException" message.
   ...
   *              exception during invocation. An exception thrown in any other
   *              case is an error.
   * @see osl.manager.Actor#send(ActorName, String)
   * @see osl.manager.Actor#call(ActorName, String)
   */
  protected abstract Object implSend(ActorMsgRequest msg)
      throws RemoteCodeException;
   ...
   * extension consists of an otherwise protected method which an implementation
   * wishes to export to its internal actor. Once registered, an extension may
   * be invoked by calling <em>implExtension</em> which attempts to look up the
   * method and make a direct call, returning any exception wrapped in an
   * <em>ExtensionException</em>.
   * <p>
   * 
   * @param <b>name</b> The name of the extension to invoke.
   * @param <b>args</b> An <em>Object[]</em> of arguments to pass to the invoked
   *        extension. If no arguments are required, this should be an array of
   *        length 0 (i.e. NOT null). The number and format of the arguments
   *        depends on the extension being invoked.
   ...
   *              Thrown as a wrapper for any error which occurs while invoking
   *              the extension.
   */
  protected Object implExtension(String name, Object[] args)
    ...
    try {
      ...
      if (potMeths == null)
        throw new NoSuchMethodException("No extension matching \"" + name
